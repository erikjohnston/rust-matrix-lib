...
fn get_dependent_state<'a>(etype: &'a str, state_key: &'a str) -> DependentState<'a> {
    match (etype, state_key) {
        ("m.room.create", "") | ("m.room.power_levels", "") => DependentState::Everything,
        ("m.room.join_rules", "") => DependentState::Memberships,
        ("m.room.member", user) => DependentState::SentBy(user),
        _ => DependentState::None,
    }
}

pub fn filter_changed_state<'a>(
    conflicted_senders: &'a StateMap<HashSet<String>>,
    delta: &[(&'a str, &'a str)],
) -> HashSet<(&'a str, &'a str)> {
    let mut entries_by_sender = HashMap::new();
    for (key, senders) in conflicted_senders.iter() {
    ...
    let mut to_recalculate: HashSet<_> = delta.iter().cloned().collect();
    let mut pending: Vec<_> = delta.iter().cloned().collect();

    let mut handled = HashSet::new();

    while let Some((t, s)) = pending.pop() {
        if handled.contains(&(t, s)) {
            continue;
        }

        handled.insert((t, s));
        if conflicted_senders.contains_key(t, s) {
            to_recalculate.insert((t, s));
            match get_dependent_state(t, s) {
                DependentState::Everything => return conflicted_senders.keys().collect(),
                DependentState::Memberships => {
                    pending.extend(
                        conflicted_senders
                            .iter_members()
                            .map(|(user, _)| ("m.room.member", user)),
                    );
                }
                DependentState::SentBy(user) => {
                    pending.extend(
                        entries_by_sender
                            .get(user)
                            .into_iter()
