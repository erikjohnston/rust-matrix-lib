...
pub fn resolve_state(
    ...
    let mut conflicted: StateMap<SmallVec<[&Event; 5]>> = StateMap::new();
    ...
    let conflicted_senders: StateMap<HashSet<String>> = conflicted
        .iter()
        .map(|(key, event_ids)| (key, event_ids.iter().map(|e| e.sender.clone()).collect()))
        .collect();
...
pub fn resolve_state_delta(
    state_sets: Vec<&StateMap<String>>,
    event_map: &HashMap<String, Event>,
    prev_conflicted_senders: &StateMap<HashSet<String>>,
    delta: &[(&str, &str)],
) -> ResolvedState {
    let to_recalculate = auth::filter_changed_state(prev_conflicted_senders, delta);
    ...
    r.conflicted_senders.extend(
        prev_conflicted_senders
            .iter()
            .filter_map(|((t, s), value)| {
...
fn resolve_conflicts_state(
    unconflicted: StateMap<String>,
    conflicted: StateMap<SmallVec<[&Event; 5]>>,
    event_map: &HashMap<String, Event>,
) -> StateMap<String> {
...
fn resolve_auth_events<'a>(
    key: (&str, &str),
    mut events: Vec<&'a Event>,
    auth_events: &StateMap<&'a Event>,
) -> &'a Event {
...
fn order_events(events: &mut Vec<&Event>) {
